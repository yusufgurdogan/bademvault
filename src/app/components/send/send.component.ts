...
export class SendComponent implements OnInit {
  ...
  amounts = [
    { name: 'BADEM (1 Badem)', shortName: 'BADEM', value: 'badem' },
  ];
  ...
  syncFiatPrice() {
    ...
    const fiatAmount = this.util.badem.rawToBdm(rawAmount).times(this.price.price.lastPrice).times(precision).floor().div(precision).toNumber();
    this.amountFiat = fiatAmount;
  }

  // An update to the fiat amount, sync the badem value based on currently selected denomination
  syncBademPrice() {
    const fiatAmount = this.amountFiat || 0;
    const rawAmount = this.util.badem.bdmToRaw(new BigNumber(fiatAmount).div(this.price.price.lastPrice));
    const bademVal = this.util.badem.rawToRaw(rawAmount).floor();
    const bademAmount = this.getAmountValueFromBase(this.util.badem.rawToRaw(bademVal));
  ...
  async sendTransaction() {
    ...
    if (from.balanceBN.minus(rawAmount).lessThan(0)) return this.notificationService.sendError(`From account does not have enough BADEM`);

    // Determine a proper raw amount to show in the UI, if a decimal was entered
    this.amountRaw = this.rawAmount.mod(this.badem);

    // Determine fiat value of the amount
    this.amountFiat = this.util.badem.rawToBdm(rawAmount).times(this.price.price.lastPrice).toNumber();
  ...
  async confirmTransaction() {
    ...
    try {
      const newHash = await this.bademBlock.generateSend(walletAccount, this.toAccountID, this.rawAmount, this.walletService.isLedgerWallet());
      if (newHash) {
  ...
  getAmountBaseValue(value) {
    ...
    switch (this.selectedAmount.value) {
      default:
      case 'badem': return this.util.badem.bdmToRaw(value);
    }
  }
  ...
  getAmountValueFromBase(value) {
    switch (this.selectedAmount.value) {
      default:
      case 'badem': return this.util.badem.rawToBdm(value);
    }
  }
