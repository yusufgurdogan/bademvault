...
export class AccountDetailsComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    this.priceSub = this.price.lastPrice$.subscribe(event => {
      this.account.balanceFiat = this.util.badem.rawToBdm(this.account.balance || 0).times(this.price.price.lastPrice).toNumber();
      this.account.pendingFiat = this.util.badem.rawToBdm(this.account.pending || 0).times(this.price.price.lastPrice).toNumber();
    });
