...
export class WalletService {
  ...
  async processStateBlock(transaction) {
    if (transaction.is_send === 'true' && transaction.block.link_as_account) {
      ...
      if (this.wallet.pending.lte(0)) {
        ...
        this.wallet.pendingFiat += this.util.badem.rawToBdm(txAmount).times(this.price.price.lastPrice).toNumber();
        this.wallet.hasPending = true;
      }
      if (this.appSettings.settings.minimumReceive) {
        const minAmount = this.util.badem.bdmToRaw(this.appSettings.settings.minimumReceive);
  ...
  reloadFiatBalances() {
    ...
    this.wallet.accounts.forEach(account => {
      account.balanceFiat = this.util.badem.rawToBdm(account.balance).times(fiatPrice).toNumber();
      account.pendingFiat = this.util.badem.rawToBdm(account.pending).times(fiatPrice).toNumber();
    });

    this.wallet.balanceFiat = this.util.badem.rawToBdm(this.wallet.balance).times(fiatPrice).toNumber();
    this.wallet.pendingFiat = this.util.badem.rawToBdm(this.wallet.pending).times(fiatPrice).toNumber();
  }
  ...
  async reloadBalances(reloadPending = true) {
    ...
    for (const accountID in accounts.balances) {
      ...
      walletAccount.balanceFiat = this.util.badem.rawToBdm(walletAccount.balance).times(fiatPrice).toNumber();
      walletAccount.pendingFiat = this.util.badem.rawToBdm(walletAccount.pending).times(fiatPrice).toNumber();
    ...
    this.wallet.pendingRaw = new BigNumber(walletPending).mod(this.badem);

    this.wallet.balanceFiat = this.util.badem.rawToBdm(walletBalance).times(fiatPrice).toNumber();
    this.wallet.pendingFiat = this.util.badem.rawToBdm(walletPending).times(fiatPrice).toNumber();
  ...
  async processPendingBlocks() {
    ...
    if (newHash) {
      ...
      const receiveAmount = this.util.badem.rawToBdm(nextBlock.amount);
      this.notifications.sendSuccess(`Successfully received ${receiveAmount.isZero() ? '' : receiveAmount.toFixed(2)} Badem!`);
